...
echo "**** Creating AUDIO data folder ****"
...
# 待修改具体数目
[ $n -ne 275756 ] && \
  echo Warning: expected 275756 data data files, found $n
...
for dir in $train_dir; do
  ...
  (echo "jieba is not found. Use tools/extra/install_jieba.sh to install it." && exit 1;)
   ...
   utils/filter_scp.pl -f 1 $dir/utt.list $text_dir | \
     ...
     python local/word_segmentation.py $dir/word_seg_vocab.txt $dir/trans_1.txt| tr '[a-z]' '[A-Z]' | sed 's/FIL/[FIL]/g' | sed 's/SPK/[SPK]/'| sort -u  > $dir/text
   ...
   utils/filter_scp.pl -f 1 $dir/utt.list $dir/utt2spk_all | sort -u | awk '{print $1" "$2}' > $dir/utt2spk
...
echo "$0: AUDIO data preparation succeeded"
exit 0;
