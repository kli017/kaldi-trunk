...
# 待修改具体数目
[ $n -ne 9999 ] && \
  echo Warning: expected 9999 data data files, found $n
...
for dir in $train_dir; do
  ...
  (echo "jieba is not found. Use tools/extra/install_jieba.sh to install it." && exit 1;)
   ...
   utils/filter_scp.pl -f 1 $dir/utt.list $text_dir | \
     ...
     python local/word_segmentation_tab.py $dir/word_seg_vocab.txt $dir/trans_1.txt > $dir/text
...
echo "$0: commonvoice data preparation succeeded"
exit 0;
