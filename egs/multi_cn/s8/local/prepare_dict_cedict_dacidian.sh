...
# phone set at the end. For Chinese, we use two online dictionary(cedict and DaCiDian),
...
mkdir $dict_dir/DaCiDian/ 
cp  /home/kli/kaldi/egs/multi_cn/s5_1/data/local/dict/DaCiDian/DaCidian_lexicon_1.txt $dict_dir/DaCiDian/

# remove dupliction of cedict from DaCiDian and merge to a new dict
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
$dict_dir/cedict/ch-dict.txt $dict_dir/DaCiDian/DaCidian_lexicon_1.txt |\
egrep -v '<.?s>' > $dict_dir/DaCiDian/DaCiDian-dict.txt 

# combine two lexicons
cat $dict_dir/cedict/ch-dict.txt $dict_dir/DaCiDian/DaCiDian-dict.txt |\
  sort -u > $dict_dir/Combined-dict-tmp.txt || exit 1;

perl -p -e 's/\r//g' $dict_dir/Combined-dict-tmp.txt > $dict_dir/Combined-dict.txt

# combine cedict and DaCiDian get oov adn in-words
echo "--- Searching for Chinese OOV words ..."
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $dict_dir/Combined-dict.txt $dict_dir/lexicon-ch/words-ch.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-ch/words-ch-oov.txt || exit 1;


awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-ch/words-ch.txt $dict_dir/Combined-dict.txt |\
...
cat $dict_dir/Combined-dict.txt |\
  perl -e '
  use utf8;
  ...
  ' > $dict_dir/Combined-dict-1.txt || exit 1;

# extract chars
cat $dict_dir/Combined-dict-1.txt | awk '{print $1}' |\
  perl -e '
  use utf8;
  binmode(STDIN,":encoding(utf8)");
  binmode(STDOUT,":encoding(utf8)");
...
cat $dict_dir/Combined-dict-1.txt |\
