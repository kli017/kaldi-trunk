...
affix=cnn_lstm_1e_3   # affix for the TDNN directory name
tree_affix=
train_stage=-10
get_egs_stage=-10
decode_iter=
...
# decode options
test_sets="aishell aidatatang magicdata thchs aishell2 audio"
#test_sets=""
test_online_decoding=true  # if true, it will run the last decoding stage.

#pre_train=exp/chain${nnet3_affix}/tdnn_cnn_1b_3_sp_new/392.mdl
...
# run those things. if you already pass through other dnn models skip this step.
...
dir=exp/chain${nnet3_affix}/tdnn_${affix}_sp_new
...
# local/chain/run_chain_common.sh, skip if you have run with other dnn models without change the nnet3_affix
...
if [ $stage -le 14 ]; then
  ...
  lstm_opts="l2-regularize=0.005 decay-time=40"
  ...
  spec-augment-layer name=idct-spec-augment freq-max-proportion=0.3 time-zeroed-proportion=0.2 time-mask-max-frames=20
  ...
  conv-relu-batchnorm-layer name=cnn1 $cnn_opts height-in=40 height-out=40 time-offsets=-3,0,3 height-offsets=-3,0,3 num-filters-out=64 learning-rate-factor=0.333 max-change=0.25
  conv-relu-batchnorm-layer name=cnn2 $cnn_opts height-in=40 height-out=40 time-offsets=-3,0,3 height-offsets=-3,0,3 num-filters-out=64
  ...
  tdnnf-layer name=tdnnf14 $tdnnf_opts dim=1536 bottleneck-dim=160 time-stride=5
  tdnnf-layer name=tdnnf15 $tdnnf_opts dim=1536 bottleneck-dim=160 time-stride=5
  tdnnf-layer name=tdnnf16 $tdnnf_opts dim=1536 bottleneck-dim=160 time-stride=5
