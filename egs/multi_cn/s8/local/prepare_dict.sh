...
# remove dupliction of cedict from DaCiDian
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
...
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  ...
  egrep -v '<.?s>' > $dict_dir/lexicon-ch/words-ch-oov-cedict.txt || exit 1;

awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $dict_dir/DaCiDian/DaCiDian-dict.txt $dict_dir/lexicon-ch/words-ch-oov-cedict.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-ch/words-ch-oov-new.txt || exit 1;


awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-ch/words-ch.txt $dict_dir/cedict/ch-dict.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-ch/lexicon-ch-iv-cedict.txt || exit 1;

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-ch/words-ch.txt $dict_dir/DaCiDian/DaCiDian-dict.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-ch/lexicon-ch-iv-dacidian.txt || exit 1;

# combine two lexicons
cat $dict_dir/lexicon-ch/lexicon-ch-iv-cedict.txt $dict_dir/lexicon-ch/lexicon-ch-dacidian.txt |\
  sort -u > $dict_dir/lexicon-ch-iv-new.txt || exit 1;
