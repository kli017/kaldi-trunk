...
aishell_text=$1/transcript/aishell_transcript_v0.8_new.txt
data=data/aishell
...
local/prepare_aishell2_dict.sh $data/local/dict || exit 1;
...
for dir in $train_dir $dev_dir $test_dir; do
  ...
  utils/filter_scp.pl -f 1 $dir/utt.list $aishell_text|\
    ...
    sed 's/ｔ/t/g' | sed 's/，//g'  |\
    sed 's/。//g'  | sed 's/？//g' > $dir/transcripts.txt

  perl -p -e 's/\r//g' $dir/transcripts.txt > $dir/trans_1.txt
  awk '{print $1}' $dict_dir/lexicon.txt | sort | uniq | awk '{print $1,99}'> $dir/word_seg_vocab.txt
  python local/word_segmentation_tab.py $dir/word_seg_vocab.txt $dir/trans_1.txt | tr '[a-z]' '[A-Z]' | sed 's/FIL/[FIL]/g' | sed 's/SPK/[SPK]/'| awk '{if (NF > 1) print $0;}' | sort -u > $dir/text
