...
utils/filter_scp.pl -f 1 $tmp/utt.list $tmp/tmp_wav.scp | sort -u  > $tmp/wav.scp
...
python local/word_segmentation_tab.py $tmp/word_seg_vocab.txt $tmp/trans.txt| sort -u  > $tmp/text

# utt2spk & spk2utt
awk -F'\t' '{print $2}' $tmp/wav.scp > $tmp/wav.list
sed -e 's:\.wav::g' $tmp/wav.list | \
  awk -F'/' '{i=NF-1;printf("%s\t%s\n",$NF,$i)}' > $tmp/tmp_utt2spk
utils/filter_scp.pl -f 1 $tmp/utt.list $tmp/tmp_utt2spk | sort -u | awk '{print $1" I"$2}' > $tmp/utt2spk
utils/utt2spk_to_spk2utt.pl $tmp/utt2spk | sort -u > $tmp/spk2utt
