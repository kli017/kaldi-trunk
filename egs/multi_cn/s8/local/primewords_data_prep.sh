...
perl -p -e 's/\r//g' $data/train/transcripts.txt > $data/train/trans_1.txt
awk '{print $1}' $dict_dir/lexicon.txt | sort | uniq | awk '{print $1,99}'> $data/train/word_seg_vocab.txt
python local/word_segmentation.py $data/train/word_seg_vocab.txt $data/train/trans_1.txt| tr '[a-z]' '[A-Z]' |awk '{if (NF > 1) print $0;}'   > $data/train/text
